...
\begin{itemize}
  ...
  \item  \textbf{One-to-One Encrypted Reads:} A ship can make data available to a
single peer ship. Response messages are authenticated via external \textsc{hmac} and internal signature. They are encrypted using a symmetric key derived from the Diffie-Hellman key exchange of the two ships' keys.
...
\begin{itemize}
  \item  The actual content path is hidden from network observers and unauthorized parties
  \item  Only the key ID is visible in plaintext.
  \item  The group key must be obtained separately (typically via a \lstinline[style=inlinecode]{%keen} to decrypt both the path and the message payload.
  \item  Different groups using different key IDs can coexist without revealing which content is being accessed.
\end{itemize}
...
\begin{enumerate}
  \item  Message encryption: The message is encrypted with XChaCha20-8 using a symmetric key derived from Curve25519 \textsc{ecdh} key exchange between the two ships' networking keys.
  \item  \textsc{hmac} authentication: A 16-byte \textsc{hmac} (BLAKE3 keyed hash) is computed over:
    \begin{itemize}
      \item  The encoded beam path.
      \item  The \textsc{lss} root of the encrypted data.
    \end{itemize}
  \item  Shared symmetric key: Both \textsc{hmac} computation and encryption use the same \textsc{ecdh}-derived symmetric key. Both parties can independently derive this key from their own private key and the other party's public key.
  \item  Verification: The receiver verifies the \textsc{hmac} using the shared symmetric key, then decrypts with the same key.
  \item  Security model: The \textsc{hmac} proves the sender possesses the shared symmetric key (implicitly authenticating them as the expected peer). No signatures are needed since only two parties share this key. This applies to both pokes and acks.
\end{enumerate}

The \lstinline[style=inlinecode]{%chum} namespace uses encrypted paths with the structure:
...
\begin{enumerate}
  \item  server-life: The life (key revision number) of the server ship's
...
safe-but-dumb ``sign every 1 KiB packet'' design of old \lstinline[style=inlinecode]{%ames}. The standard approach is to use asymmetric crypto to establish a shared \textsc{aead} key, and use it to authenticate each packet. This is just as safe as signing every packet, and orders of magnitude faster. However, we still can't verify that a peer is sending us \emph{correct} data until we've received the entire message. They could send us 999 good packets and 1 bad one, and we'd have no way of knowing which was which. This is especially annoying if we want to download in parallel from multiple peers: if the final result is invalid, which peer is to blame? If we want to solve this problem, we need to get a little more bespoke.
